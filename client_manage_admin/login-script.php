...
if($_SERVER["REQUEST_METHOD"]=="POST") {
	...
	$sql = "SELECT * FROM client_admin_users WHERE client_email = '$client_email' AND client_mobile = '$client_mobile' AND status=0";
	$result = $conn->query($sql);
	$row = $result->fetch_assoc();

	if($row) {
	    $_SESSION['client_admin_user_id'] = $row['id'];
	    $_SESSION['client_admin_user_name'] = $row['client_name'];
	    $_SESSION['client_admin_user_flag'] = 0;
	    //Assign the current timestamp as the user's
		//latest activity
		$_SESSION['last_action'] = time();
	    if(isset($_SESSION["client_admin_user_name"])) {
